...
#define MIPRTPGSMENCODER_ERRSTR_BADMESSAGE			"Can't understand message"
#define MIPRTPGSMENCODER_ERRSTR_NOTINIT				"RTP GSM encoder not initialized"

MIPRTPGSMEncoder::MIPRTPGSMEncoder() : MIPRTPEncoder("MIPRTPGSMEncoder")
{
	m_init = false;
}

MIPRTPGSMEncoder::~MIPRTPGSMEncoder()
{
	cleanUp();
}

bool MIPRTPGSMEncoder::init()
{
	if (m_init)
		cleanUp();
...
bool MIPRTPGSMEncoder::push(const MIPComponentChain &chain, int64_t iteration, MIPMessage *pMsg)
{
	if (!m_init)
	{
		setErrorString(MIPRTPGSMENCODER_ERRSTR_NOTINIT);
		return false;
	}
	...
	{
		setErrorString(MIPRTPGSMENCODER_ERRSTR_BADMESSAGE);
		return false;
	}
...
bool MIPRTPGSMEncoder::pull(const MIPComponentChain &chain, int64_t iteration, MIPMessage **pMsg)
{
	if (!m_init)
	{
		setErrorString(MIPRTPGSMENCODER_ERRSTR_NOTINIT);
...
void MIPRTPGSMEncoder::cleanUp()
{
	clearMessages();
	m_init = false;
}

void MIPRTPGSMEncoder::clearMessages()
{
